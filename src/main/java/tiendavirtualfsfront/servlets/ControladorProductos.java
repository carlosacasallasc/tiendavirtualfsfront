...
import tiendavirtualfsfront.modelo.Proveedores;
import tiendavirtualfsfront.json.ProveedoresJSON;
			...
			if(is.available() ==0) {
				...
				if(ProductosJSON.validarCSV(nomb)) {
					try {
						...
						productos.add(new Productos(Long.parseLong(codigo), nombre, Long.parseLong(nit), Double.parseDouble(compra), Double.parseDouble(iva), Double.parseDouble(venta)));
						}
						is.close();
						...
						for(Productos item:listafinal) {
							...
							for(Productos item2:listaproductos) {
								if(item.getCodigoProducto()!=item2.getCodigoProducto()) {
									validacion = ProductosJSON.postJSON(item);
									if(validacion==200) {
										ban = (ban * 0) + 2;
									}
								}else {
									registros2 = String.valueOf(item.getCodigoProducto())+",";
								}
							}		
						}
						if(validacion == 200) {
